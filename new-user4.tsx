...
function UserInsertPage() {
  ...
  const roleLabelById = (id: number | null) =>
    id == null ? "-" : ROLE_OPTIONS.find((role) => role.id === id)?.label ?? "-";

  const deptLabelById = (id: number | null) =>
    id == null ? "-" : DEPT_OPTIONS.find((dept) => dept.id === id)?.label ?? "-";
  ...
  const handleInsert = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!Number.isInteger(parsedRoleId) || !Number.isInteger(parsedDeptId) || !trimmedPassword || !trimmedFullName) {
      setMessage({
        type: "error",
        text: "Please select role and department, and fill in password and full name.",
      });
      setLoading(false);
      return;
    }
      ...
      <div className="mx-auto max-w-2xl rounded-2xl border border-white/10 bg-white/5 p-6 shadow-[0_18px_70px_rgba(15,23,42,0.55)] backdrop-blur">
        ...
        {lastInserted && (
          <div className="mt-4 rounded-lg border border-cyan-300/30 bg-cyan-300/10 p-3 text-sm text-cyan-100">
            ...
            <p>
              Role: {lastInserted.role_id ?? "-"} {lastInserted.role_id != null ? `(${roleLabelById(lastInserted.role_id)})` : ""}
            </p>
            <p>
              Department: {lastInserted.dept_id ?? "-"}{" "}
              {lastInserted.dept_id != null ? `(${deptLabelById(lastInserted.dept_id)})` : ""}
            </p>
